...
static 
int 
PL_get_name_arity_spec(term_t A1, atom_t *name, size_t *arity  ARG_LD)
{
  int RC = PL_get_name_arity(A1,name,arity);
  if(RC!=TRUE)
  { return RC;     
  }
  int dcgpi = (*name==ATOM_gdiv);
  if(*arity==2 && (dcgpi || *name==ATOM_divide))
  { term_t a = PL_new_term_ref();
    ...
    { *arity = 0;
      return TRUE;
    } else if (!PL_get_size_ex(a,arity))
    { return FALSE;
    } else 
    {
      if(dcgpi) *arity+=2;
...
static unsigned int
current_metaterm_mask(atom_t key)
{ matts_flag *index;
...
static atom_t
current_caller_mask(unsigned int mask)
{ matts_flag *index;
...
{ GET_LD
  ...
  { int value = current_metaterm_mask(bitname);
...
static
PRED_IMPL("current_metaterm_mask", 2, current_metaterm_mask, PL_FA_NONDETERMINISTIC)
{ PRED_LD
  ...
  { case FRG_FIRST_CALL:
      ...
      { int value = current_metaterm_mask(bt);
...
{ if(arity>MAX_ARITY || arity < 0 || current_name==ATOM_dmetaterm_call) return ATOM_false;
  ...
  { if(found->name==0) 
    ...
    if(found->name==current_name) 
    {
      ...
      if( found->arity==arity ) return found->value;
    }
...
{ if(arity>MAX_ARITY || arity<0 || current_name==ATOM_dmetaterm_call) return;
  ...
  { if(found->name==0) {  if(!lastFree) lastFree = found; break; }
    ...
    if(found->name==current_name) 
    {
      if( found->arity==0 ) { lastFree = found; break; }
      if( found->arity==arity ) { lastFree = found; break; }
    }
	if (--maxsize <= 0) { trap_gdb(); return;}
    found++;
  }
  assert(lastFree);
  found = lastFree;
  if(value==ATOM_unknown) 
  { if(found->name==0) return;
    found->name=ATOM_nil;
    return;
  }
  if(found->name==0) 
  ...
  { if(found->arity==0) 
    {
      found->arity = arity;
      addOverriden(current_name, 0, found->value PASS_LD);
    } else
    {
      found->arity = 0;
    }
...
assignAttVarBinding(Word av, Word value, int flags ARG_LD)
{
 ...
 DEBUG(CHK_SECURE, assert(on_attvar_chain(av)));
  ...
  { if ( av == value ) return;
    ...
    { if ( !IS_META(METATERM_DISABLE_SWAP) )
      { DEBUG(MSG_ATTVAR_GENERAL, Sdprintf_ln("SWAPPING value <- av "));
        *value = makeRef(av);
        return;
      } else
      { DEBUG(MSG_METATERM, Sdprintf_ln("METATERM_DISABLE_SWAP value -> av "));
        if ( IS_META(METATERM_NO_BIND) ) {Sdprintf_ln("Yet METATERM_NO_BIND "); }else *av = makeRef(value);
        return;
      }
    }
...
assignAttVar(Word* avP, Word* valueP, int callflags ARG_LD)
{
  ...
  if(isVar(*value))
  {
    ...
    { if(atomcaller==0) atomcaller = current_caller_mask(callflags);
      ...
      DEBUG(MSG_WAKEUPS, Sdprintf_ln("METATERM_USE_UNIFY_VAR(%s,%s)", vName(av),my_atom_summary(atomcaller)));
      registerWakeup(FUNCTOR_pre_unify5, atomcaller, av, valPAttVar(*av), value PASS_LD);
      return TRUE;
    }
 ...
 { if(!(flags& METATERM_NO_TRAIL)) TrailAssignment(av);
 }
...
static int
find_sub_attr(Word l, functor_t name, Word *vp ARG_LD)
{ for(;;)
  { if ( isNil(*l) )
    { *vp = l;
      fail;
...
{ GET_LD
  ...
  if(PL_get_atom(opval, &math))
  {
      int value;
        ...
        {
           value = current_metaterm_mask(math);
        ...
        int maskwas = current_metaterm_mask(math);
        if(maskwas)
        {
...
{ PRED_LD
  return metaterm_flags(A1,A2,A3,METATERM_NO_INHERIT, NB_PUTATTS PASS_LD);
}

static
PRED_IMPL("metaterm_flags", 3, metaterm_flags, 0)
{ PRED_LD
  return metaterm_flags(A1,A2,A3,METATERM_NO_INHERIT,  B_PUTATTS PASS_LD);
}
...
*/
int
fvOverride(atom_t method, Word attvar, Word value, int* retresult ARG_LD)
...
{ PRED_LD
  size_t		arity;
  atom_t		name, value; 
  int RC = PL_get_name_arity_spec(A1,&name,&arity PASS_LD);
  if(!RC) return RC;
 	RC = PL_get_atom_ex(A2,&value);
 	if(!RC) return RC;
 	addOverriden(name,arity,value PASS_LD);
 	return TRUE;
}
...
{ PRED_LD
  ...
  int RC = PL_get_name_arity_spec(A1,&name,&arity PASS_LD);
  if(!RC) return RC;
  return PL_unify_atom(A2,getPredOverriden(name,arity PASS_LD));
}
...
#define addNeverOverriden(N,A) addOverriden(N,A, ATOM_false PASS_LD)
#define addSometimesOverriden(N,A) addOverriden(N,A, ATOM_true PASS_LD)
...
setupMetaterms(ARG1_LD)
{
  ...
  {
    addNeverOverriden(PL_new_atom(from2->name), 0);
      from2++;
  }
    addNeverOverriden(ATOM_dmeta, 0) ;
    addNeverOverriden(ATOM_dmetaterm_call ,0 );
    addNeverOverriden(ATOM_dmetaterm_callp , 0 );
    addSometimesOverriden(ATOM_strict_equal, 2 );
    addSometimesOverriden(ATOM_compare,3  );
    addSometimesOverriden(ATOM_copy_term, 2 );
    addSometimesOverriden(ATOM_at_equals, 2 );
    addSometimesOverriden(ATOM_at_not_equals,2 );
