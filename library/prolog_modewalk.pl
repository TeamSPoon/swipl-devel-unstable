...
:- module(prolog_modewalk,
	  [ prolog_walk_mode/1,		% +Options
	    prolog_program_mode_clause/2	% -ClauseRef, +Options
	  ]).
...
knowable(O):-knowable(O,_).

knowable(O, _):-var(O),!,fail.
knowable(O,OO):-callable(O),!,OO=O.
knowable(O,OO):-format(atom(OA),'callable_~w',[O]),OA=OO.


/** <module> Prolog mode walker

This module walks over  the  loaded   program,  searching  for  knowable
...
source file is passed into _Where.
  ...
  assert_call_graph :-
	  ...
	  prolog_walk_mode([ trace_reference(_),
...
:- mode(prolog_walk_mode(:)).

:- multifile
	prolog:called_by/4,
	prolog:called_by/2.

:- predicate_options(prolog_walk_mode/1, 1,
		     [ undefined(oneof([ignore,error,trace])),
		       ...
		       trace_reference(any),
		       on_trace(knowable),
		       infer_modes(oneof([false,true,all])),
		       evaluate(boolean)
		     ]).
...
:- record
	walk_option(undefined:oneof([ignore,error,trace])=ignore,
		    ...
		    infer_modes:oneof([false,true,all])=true,
		    clauses:list,		% Walk only these clauses
		    trace_reference:any=(-),
		    on_trace:knowable,		% Call-back on trace hits
						% private stuff
...
%%	prolog_walk_mode(+Options) is det.
%
%	Walk over all loaded (user) Prolog mode. The following mode is
%	processed:
%
...
%
%	  * autoload(+Boolean)
%	  Try to autoload mode while walking. This is enabled by default
...
%
%	  * infer_modes(+BooleanOrAll)
%	  Use infer_mode/2 on predicates with clauses that
...
%	  knowable argument.  If =all=, it will be restarted until no
...
prolog_walk_mode(Options) :-
	meta_options(is_meta, Options, QOptions),
	prolog_walk_mode(1, QOptions).

prolog_walk_mode(Iteration, Options) :-
	...
	infer_new_modes(New, OTerm),
	statistics(cputime, CPU1),
	(   New \== []
	->  CPU is CPU1-CPU0,
	    print_message(informational,
			  modewalk(reiterate(New, Iteration, CPU))),
	    succ(Iteration, Iteration2),
	    prolog_walk_mode(Iteration2, Options)
	;   true
	).
...
no_walk_property(foreign).		% cannot walk foreign mode
...
walk_called(Meta, M, term_position(_,E,_,_,ArgPosList), OTerm) :-
	...
	(   predicate_property(M:Meta, mode(Head))
	;   inferred_mode(M:Meta, Head)
	), !,
	walk_option_clause(OTerm, ClauseRef),
...
walk_called(Goal, _Module, TermPos, OTerm) :-
	not_knowable(Goal, TermPos, OTerm).
...
%%	not_knowable(+Goal, +TermPos, +OTerm)
%
%	We found a reference to a non-knowable term

not_knowable(Goal, TermPos, OTerm) :-
	print_reference(Goal, TermPos, not_knowable, OTerm).
...
make_message(undefined, Goal, Context,
	...
	goal_pi(Goal, PI).
make_message(not_knowable, Goal, Context,
	     error(type_error(knowable, Goal), Context), error).
...
register_possible_meta_clause(ClausesRef) :-
	...
	\+ predicate_property(Module:Head, mode(_)),
	\+ inferred_mode(Module:Head, _),
	\+ possible_mode(Head, Module), !,
	assertz(possible_mode(Head, Module)).
...
%%	infer_new_modes(-MetaSpecs, +OTerm) is det.

infer_new_modes([], OTerm) :-
	walk_option_infer_modes(OTerm, false), !.
infer_new_modes(MetaSpecs, OTerm) :-
	findall(Module:MetaSpec,
		( retract(possible_mode(Head, Module)),
		  infer_mode(Module:Head, MetaSpec),
		  (   walk_option_infer_modes(OTerm, all)
		  ->  true
...
%
%	Walk mode explicitly mentioned to  be   called  through the hook
%	prolog:called_by/2.
...
extend(Goal0, N, GoalEx, TermPosIn, TermPosOut, _) :-
	knowable(Goal0,Goal), !,
	Goal =.. List,
	length(Extra, N),
...
%%	prolog_program_mode_clause(-ClauseRef, +Options) is nondet.
%
%	True when ClauseRef is a reference   for  clause in the program.
%	Options   is   a   subset   of    the   options   processed   by
%	prolog_walk_mode/1. The logic for deciding   on which clauses to
%	enumerate is shared with prolog_walk_mode/1.
%
%	  * module(?Module)
%	  * module_class(+list(Classes))

prolog_program_mode_clause(ClauseRef, Options) :-
	make_walk_option(Options, OTerm, _),
	setup_call_cleanup(
	    true,
...
prolog:message_location(file(Path, Line, _, _)) -->
	[ '~w:~d: '-[Path, Line] ].
prolog:message(modewalk(reiterate(New, Iteration, CPU))) -->
