...
dra_call_tabled(Goals ) :-
      '$dratz':dra_must(b_getval('$tabling_exec',dra_state(Stack, Hyp, ValOld, CuttedOut))),
      setup_call_cleanup(
        ((ValOld < 0) -> (( init_dra_call,EXIT = exit_dra_call )); (EXIT = cont_dra_call)),
        ((      
            Level is ValOld +1,
            dra_call_tabled(Cutted, Goals, Stack, Hyp, Level ),
            ((var(Cutted);((trace),'$dratz':non_cutted(Goals,Cutted,(CuttedOut))))->true;(!,fail)),
             EXIT)),
       ((EXIT))).
...
dra_call_coind0(Goals ) :-
      '$dratz':dra_must(b_getval('$tabling_exec',dra_state(Stack, Hyp, ValOld, CuttedOut))),
      setup_call_cleanup(
        ((ValOld < 0) -> (( '$dratz':init_dra_call,EXIT = exit_dra_call )); (EXIT = cont_dra_call)),
        ((
           % empty_hypotheses( Hyp ),
           % empty_stack( Stack ),            
            Level is ValOld +1,
