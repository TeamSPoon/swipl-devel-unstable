...
/** Module 
   ...
   somewhat different than in [2]: we use "drat_pioneer" for "topmost looping goal"),
   ...
   -- answer( goal, fact )
           ...
           may have an entry in "drat_completed" (see below).
           ...
           drat_result, i.e., the successful instantiation of that goal (which need
   ...
   -- drat_pioneer( goal, index )

           If the current goal is tabled, and it is not a variant of any of its
           ancestors, then the goal is called a "drat_pioneer" and obtains an "index"
           (i.e., an unique identifier). Both the goal and its index are
           recorded in this table.

           The role of a drat_pioneer is to compute the fixpoint (by tabling answers)
           ...
           If a drat_pioneer is later determined not to be the "topmost looping goal"
           in a "cluster" of interdependent goals (see ref. [2]), then it loses
           the status of a drat_pioneer, and its role will be overtaken by the
           topmost goal in the cluster.  (This can happen if one of the
           descendants of a drat_pioneer turns out to be a variant of one of its
           ancestors.)

           A drat_pioneer also loses its status if its fixpoint has been computed: it
           then becomes a "drat_completed" goal (and all its variants become
           drat_completed).

           A drat_pioneer "G" may also lose its status because another goal "G'",
           encountered after "G" succeeds without yet becoming drat_completed, has
           become drat_completed: if "G'" is a variant of "G", thne "G" becomes
           drat_completed as well.

           When a drat_pioneer loses its status, the associated entries in "drat_pioneer",
           "drat_loop" and "drat_looping_alternative" (see below) are removed.  The
           associated entries in "drat_result" are not removed. The unique index is
           ...
           (NOTE: In the actual implementation each fact in "drat_pioneer" has the
                  form
                     drat_pioneer( cgoal, goal, index )
                  where "cgoal" is a copy of "goal" (no shared variables),
                  passed through essence_hook/2.
                  This is done to facilitate more effective filtering (via
                  unification) before a check is made for whether "goal" is a
                  variant of the goal for which we are checking whether it is
                  (still) a drat_pioneer.
           )

   -- unique_index

           This is a non-logical variable that holds the index to be used for
           the next entry in "drat_pioneer".  It is also used to generate unique
           indices for coinductive goals, which might need them to hold their
           own results in "drat_result".

           The variable is cleared before the evaluation of a new query.


   -- drat_result( index, fact )

           A tabled goal "G" that "started out" as a drat_pioneer may have associated
           entries (marked with the unique index of "G") in "drat_result".  This
           ...
           When a goal loses its drat_pioneer status (because it is determined to be
           a part of a larger drat_loop, or because it has become drat_completed), the
           associated entries in "drat_result" are not removed.  They are removed
   ...
   -- drat_loop( index, list of goals )

           A drat_loop is discovered when the current tabled goal is a variant of one
           of its ancestors.  If the ancestor is a drat_pioneer, the unique index of
           the drat_pioneer ancestor and a list of the tabled goals between the
           drat_pioneer and the variant are stored in "drat_loop".

           A number of "drat_loop" entries may exist for a given drat_pioneer: together,
           they describe a "cluster" (i.e., a "strongly connected component",
           see ref. [1]).  Before finally failing upon backtracking, a drat_pioneer
           will compute its own fixpoint as well as the fixpoints of the goals
           in its cluster.

           When a goal loses its drat_pioneer status (because it is determined to be
           a part of a larger drat_loop, or because it has become drat_completed), the
           associated entries in "drat_loop" are removed.

           This table is cleared before the evaluation of a new query.


   -- drat_looping_alternative( index, clause )

           When a goal "G" is determined to be a variant descendant of a
           drat_pioneer, the clause that is currently being used by the drat_pioneer
           (i.e., the clause that led to "G") is stored in this table, together
           with the unique index of the drat_pioneer.  "G" will then succeed only
           ...
           drat_pioneer.  (This is the essence of the "dynamic reordering of
           alternatives".
            )

           When a goal loses its drat_pioneer status (because it is determined to be
           a part of a larger drat_loop, or because it has become drat_completed), the
           associated entries in "drat_looping_alternative" are removed.
   ...
   -- drat_completed( goal )
           ...
           Indicates that this tabled goal is drat_completed, i.e., its fixpoint has
           been computed, and all the possible results for variants of the goal
           can be found in table "answer".  Variants of a drat_completed goal are
           drat_completed as well.

           This table is not cleared before the evaluation of a new query.

           (NOTE: In the actual implementation each fact in "drat_completed" has the
                  form
                     drat_completed( cgoal, goal )
                  where "cgoal" is a copy of "goal" (no shared variables),
                  passed through essence_hook/2.
                  This is done to facilitate more effective filtering (via
                  unification) before a check is made for whether "goal" is a
  ...
  NOTE:
    ...
    3. Just before the drat_result of a query is reported, the interpreter
    ...
    5. The program may contain clauses that modify the definition of the
       ...
       will drat_result in "p( a, b, c )" and "p( a, b, d )" being treated as
     ...
     The metainterpreter should provide the following predicates
        ("hooks") that will be called by the top level:
           ...
           - dra_call_interp/1:
                  ...
                  "dra_call_interp( Q )".  Depending on the drat_result, it will then
...
% A tabled goal that has been drat_completed: all the results are in "answer".

dra_call_tabled(_Cutted, Goal, _, _, Level ) :-
        is_completed_variant( Goal ),
        !,
        dra_incval_flag( step_counter ),
        trace_entry( drat_completed, Goal, '?', Level ),
        (
            get_all_tabled_answers( Goal, '?', drat_completed, Level )
        ;
            trace_failure( drat_completed, Goal, '?', Level ),
            fail
        ).
...
%
%       2. Ancestor drat_pioneer goals between this goal and its variant ancestor
%          will lose their status as pioneers (and the associated entries in
%          "drat_loop" and "drat_looping_alternative" will be removed).
%
%       3. If the variant ancestor is a drat_pioneer, then:
%             - the entire prefix of the list of goals upto (but not including)
%               the variant ancestor will be added to the cluster of that
%               ancestor (by storing it in "drat_loop");
...
%               "drat_looping_alternative" entries for that ancestor.
%
%       4. If this goal is coinductive, then we use "drat_result" to avoid
%          duplicating results.
...
dra_call_tabled(_Cutted, Goal, Stack, Hyp, Level ) :-
        ...
        % Create a looping alternative if the variant ancestor is a drat_pioneer:
        (
        ...
        ->
            ...
            ;
                ...
                retractall( drat_result( PGIndex, _ ) ),
                fail
            )
        ;
...
% A drat_pioneer goal is called by program clauses, producing results that are stored
% in "answer".
...
% of "drat_looping_alternative" will be used to produce more answers (but only those
% that have not yet been produced by the goal), until a fixed point is reached.
% The drat_pioneer (and all the goals in its cluster) will then be marked as
% complete, and will cease to be a drat_pioneer.
%
% Note that a drat_pioneer may also lose its status when some descendant goal finds
% a variant ancestor that is also an ancestor of the drat_pioneer.  See the case
% of "variant of ancestor" above.
%
% Note also that a goal might become drat_completed after it succeeded (because
% another variant goal "on the right" has drat_completed), so after backtracking it
...
dra_call_tabled(Cutted, Goal, Stack, Hyp, Level ) :-
        ...
        trace_entry( drat_pioneer, Goal, PGIndex, Level ),

        (
            get_tabled_if_old_first( Goal, PGIndex, drat_pioneer, Level )
        ;

            NLevel is Level +1,
            use_clause(Goal, Body ),
            \+ is_completed_variant( OriginalGoal ), % might well be, after backtracking
            ...
            trace_success( drat_pioneer, Goal, PGIndex, Level )
        ;

            % All the clauses have been exhausted, except for looping
            % alternatives (if any).  However, the goal may have become
            % drat_completed (by a later variant), or it might have lost its drat_pioneer
            % status (because it belongs to a larger drat_loop).

            is_completed_variant( Goal )                      % a variant has drat_completed?
        ->
            trace_other( 'Removing drat_completed drat_pioneer', Goal, PGIndex, Level ),
            rescind_pioneer_status( PGIndex ),
            get_remaining_tabled_answers( Goal, PGIndex, 'drat_completed now', Level )
        ;

            is_a_variant_of_a_pioneer( Goal, PGIndex )  % not lost drat_pioneer status?
        ->
            (
            ...
            ;
                ...
                trace_other( 'Removing drat_pioneer', Goal, PGIndex, Level ),
                rescind_pioneer_status( PGIndex ),
                get_remaining_tabled_answers( Goal, PGIndex,
                                              'drat_completed now', Level
                                            )
            ;
                retractall( drat_result( PGIndex, _ ) ),
                fail
        ...
        ;
            ...
            (
                % No longer a drat_pioneer and not drat_completed, so just sequence
                % through the remaining available tabled answers.
                get_remaining_tabled_answers( Goal, PGIndex,
                                              '(no longer a drat_pioneer)', Level
                                            )
            ;
                trace_failure( '(no longer a drat_pioneer)', Goal, PGIndex, Level ),
                retractall( drat_result( PGIndex, _ ) ),
                fail
            )
        ).
...
% remembering them in "drat_result", then succeed; otherwise just fail.
...
%                             ):
% Return (one by one) all the answers that are currently tabled for this goal
% but are not present in its "drat_result" entries.
% (Each answer is returned by appropriately instantiating the goal.)
...
% compute_fixed_point( +drat_pioneer goal, +its index, +stack, +level ):
% Solve the goal by associated rules from "drat_looping_alternative", succeeding
...
suppress_pioneers_on_list( Triples, Level ) :-
        ...
        trace_other( 'Removing drat_pioneer', M, MI, Level ),
...
% Remove auxiliary table entries for the drat_pioneer with this index.
% Specifically, clean up "drat_pioneer", "drat_loop" and "drat_looping_alternative".
...
% complete_cluster( +PioneerIndex of a drat_pioneer goal, +TraceLevel ):
% If the goal has an associated cluster, make sure all the goals in the cluster
% are marked as drat_completed.
...
complete_cluster( PGIndex, Level ) :-
        ...
        member( G, Gs ),                        % iterate over members of a drat_loop
...
%       the tables "answer", "drat_result", "drat_pioneer", "drat_loop",
%       "drat_looping_alternative" and "drat_completed".
...
:-dynamic drat_pioneer/3 .
:-dynamic drat_result/2 .
:-dynamic drat_loop/2 .
:-dynamic drat_looping_alternative/2 .
:-dynamic drat_completed/2 .
...
print_tables :-
       ...
       listing( drat_result( _, _ ) ),
       listing( drat_pioneer( _, _, _ ) ),
       listing( drat_loop( _, _ ) ),
       listing( drat_looping_alternative( _, _ ) ),
       listing( drat_completed( _, _ ) ).
...
reinitialise_result :-
        retractall( drat_result( _, _ ) ).


% '$is_result_known'( +index, +fact ):
% Does the table "drat_result" contain a variant of this fact associated with this
% index?

% :-mode '$is_result_known'( +, +).

'$is_result_known'( PGIndex, Fact ) :-
        drat_result( PGIndex, F ),
        are_essences_variants( F, Fact ),
        !.


% new_result_or_fail( +index, +fact ):
% If the table "drat_result" already contains a variant of this fact associated with
...
new_result_or_fail( PGIndex, Fact ) :-
        \+ '$is_result_known'( PGIndex, Fact ),
        assert( drat_result( PGIndex, Fact ) ).
...
reinitialise_pioneer :-
        retractall( drat_pioneer( _, _, _ ) ).

% is_a_variant_of_a_pioneer( +goal, -index ):
% Succeeds if the goal is a variant of a goal that is tabled in "drat_pioneer";
% returns the index of the relevant entry in table "drat_pioneer".
...
is_a_variant_of_a_pioneer( Goal, PGIndex ) :-
        copy_term( Goal, Copy ),
        once( essence_hook( Copy, CopyEssence ) ),
        drat_pioneer( CopyEssence, G, PGIndex ),
        are_essences_variants( Goal, G ),
        !.


% add_pioneer( +goal, -index ):
% Add an entry for this goal to "drat_pioneer", return the unique index.
...
add_pioneer( Goal, PGIndex ) :-
        ...
        once( essence_hook( Copy, CopyEssence ) ),
        get_unique_index( PGIndex ),
        assert( drat_pioneer( CopyEssence, Goal, PGIndex ) ).



% delete_pioneer(+PGIndex).
% Remove the entry in "drat_pioneer" associated with this index.

delete_pioneer( PGIndex ) :-
        retract( drat_pioneer( _, _, PGIndex )).
...
reinitialise_loop :-
        retractall( drat_loop( _, _ ) ).


% add_loop( +index, +list of goals ):
% Add an entry to "drat_loop".
...
add_loop( PGIndex, Goals ) :-                    % neither are duplicates
        drat_loop( PGIndex, Gs ),
        are_variants( Goals, Gs ),
        !.

add_loop( PGIndex, Goals ) :-
        assert( drat_loop( PGIndex, Goals ) ).


% delete_loops( +index ):
% Remove all the entries in "drat_loop" that are associated with this index.

delete_loops( PGIndex ) :-
        retractall( drat_loop( PGIndex, _ ) ).


% get_loop( +index, -Goals ):
% Get an entry from table "drat_loop" that is associated with this index;
...
delete_looping_alternatives( PGIndex ) :-
        retractall( drat_looping_alternative( PGIndex, _ ) ).


% get_looping_alternative( +index, -clause ):
% Get an entry from table "drat_looping_alternative" that is associated with this
% index; another such entry (if it exists) on backtracking etc.

get_looping_alternative( PGIndex, Clause ) :-
        drat_looping_alternative( PGIndex, Clause ).
...
% reinitialise_completed:
% Clear the table of drat_completed goals.

reinitialise_completed :-
        retractall( drat_completed( _, _ ) ).


% is_completed_variant( +goal ):
% Succeeds iff the goal is a variant of a goal that has been stored in
% the table "drat_completed".

% :-mode is_completed_variant( +).

is_completed_variant( Goal ) :-
        copy_term( Goal, Copy ),
        once( essence_hook( Copy, CopyEssence ) ),
        drat_completed( CopyEssence, G ),
        are_essences_variants( Goal, G ).


% complete_goal( +goal, +index for tracing ):
% Make sure the goal is marked as drat_completed.
...
complete_goal( Goal, Level ) :-
        ...
        assert( drat_completed( CopyEssence, Goal ) ).
...
% Each item recorded for table "drat_result" is of the form "drat_result( Fact )".
% The item is recorded under the key "Index".  The index is additionally
% recorded under the key "result_key".

% reinitialise_result:
% Clear the drat_result table.
reinitialise_result :-
        recorded( result_key, Index, RefIndex ),
        erase( RefIndex ),
        recorded( Index, drat_result( _ ), RefResult ),
        erase( RefResult ),
        fail.
...
new_result_or_fail( Index, Fact ) :-
        \+ '$is_result_known'( Index, Fact ),
        recordz( Index, drat_result( Fact ) ),
...
% Each item recorded for table "drat_pioneer" is of the form
% "drat_pioneer( Filter, Goal, Index )" (where "Index" is unique and "Filter" is the
...
reinitialise_pioneer :-
        ...
        erase( RefIndex ),
        recorded( Key, drat_pioneer( _, _, _ ), RefResult ),
        erase( RefResult ),
        fail.
...
is_a_variant_of_a_pioneer( Goal, Index ) :-
        copy_term( Goal, Copy ),
        once( essence_hook( Copy, CopyEssence ) ),
        recorded( Goal, drat_pioneer( CopyEssence, G, Index ) ),
        are_essences_variants( Goal, G ),
        !.
...
add_pioneer( Goal, Index ) :-
        ...
        copy_term( Goal, Copy ),
        once( essence_hook( Copy, CopyEssence ) ),
        recordz( Goal, drat_pioneer( CopyEssence, Goal, Index ) ),
        most_general_instance( Goal, Key ),
...
delete_pioneer( Index ) :-
        ...
        erase( RefIndex ),
        recorded( Key, drat_pioneer( _, _, Index ), RefPioneer ),
        erase( RefPioneer ).
...
% Each item recorded for table "drat_loop" is of the form "drat_loop( Goals )".
% The item is recorded under the key "Index", where "Index" is the index
% associated with the drat_loop.
...
delete_loops( Index ) :-
        recorded( Index, drat_loop( _ ), RefLoop ),
        erase( RefLoop ),
        fail.
...
get_loop( Index, Gs ) :-
        recorded( Index, drat_loop( Gs ) ).



%-------------------------------------------------------------------------------

% Each item recorded for table "drat_looping_alternative" is of the form
% "drat_looping_alternative( Clause )".
% The item is recorded under the key "Index", where "Index" is the index
% associated with the drat_looping_alternative.
...
delete_looping_alternatives( Index ) :-
        recorded( Index, drat_looping_alternative( _ ), RefLoop ),
        erase( RefLoop ),
        fail.
...
add_looping_alternative( Index, Clause ) :-
        recordz( Index, drat_looping_alternative( Clause ) ),
...
get_looping_alternative( Index, Clause ) :-
        recorded( Index, drat_looping_alternative( Clause ) ).



%-------------------------------------------------------------------------------

% Each item recorded for table "drat_completed" is of the form
% "drat_completed( Filter, Goal )", where "Filter" is the essence of a copy of
% "Goal".
% The item is recorded under the key "Goal" , i.e., effectively the key is the
% principal functor of the goal.  A most general instance of the goal is
...
reinitialise_completed :-
        ...
        erase( RefIndex ),
        recorded( Key, drat_completed( _, _ ), RefResult ),
        erase( RefResult ),
        fail.
...
is_completed_variant( Goal ) :-
        copy_term( Goal, Copy ),
        once( essence_hook( Copy, CopyEssence ) ),
        recorded( Goal, drat_completed( CopyEssence, G ) ),
        are_essences_variants( Goal, G ).
...
complete_goal( Goal, Level ) :-
        ...
        copy_term( Goal, Copy ),
        once( essence_hook( Copy, CopyEssence ) ),
        recordz( Goal, drat_completed( CopyEssence, Goal ) ),
        most_general_instance( Goal, Key ),
